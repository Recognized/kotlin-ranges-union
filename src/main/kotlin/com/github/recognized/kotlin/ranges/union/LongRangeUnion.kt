...
class LongRangeUnion {
  ...
  operator fun contains(other: LongRange): Boolean {
    ...
    val endPos = myRanges.binarySearch(LongRange.from(other.endInclusive, 1), LongRange.INTERSECTS_CMP)
  ...
  fun exclude(range: LongRange) {
    ...
    val endPos = myRanges.binarySearch(LongRange.from(range.endInclusive, 1), LongRange.INTERSECTS_CMP)
    ...
    if (!toEdit.isEmpty()) {
      ...
      if (oldEnd > range.endInclusive) {
        toEdit.add(LongRange(range.endInclusive + 1, oldEnd))
      }
    }
  }
  ...
  fun union(range: LongRange) {
    ...
    val endPos = myRanges.binarySearch(LongRange.from(range.endInclusive + 1, 1), LongRange.INTERSECTS_CMP)
    val lastTouched = if (endPos < 0) toInsertPosition(endPos) - 1 else endPos
    val toEdit = myRanges.subList(toInsertPosition(startPos), lastTouched + 1)
    if (!toEdit.isEmpty()) {
      val oldStart = toEdit[0].start
      ...
      // Clear all touched
      toEdit.clear()
      toEdit.add(LongRange(min(oldStart, range.start), max(oldEnd, range.endInclusive)))
    } else {
      toEdit.add(range)
    }
  }
  ...
  fun impose(range: LongRange): LongRange {
    ...
    val rightPart = LongRange(0, range.endInclusive)
